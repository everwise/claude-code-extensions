...
2. Gather the diff yourself with the **Bash** tool (e.g. `git diff <base>...HEAD`, default base `main`) and **Read** the changed files. Do NOT slash-invoke the `thermo-nuclear-review` or `thermo-nuclear-code-quality-review` skills — they are rubric files the subagents read from disk, not commands.
